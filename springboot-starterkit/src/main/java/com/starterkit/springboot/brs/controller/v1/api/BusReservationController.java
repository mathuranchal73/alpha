...
public class BusReservationController {
    ...
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    public ResponseEntity<?> getTripsByStops(@RequestBody @Valid GetTripSchedulesRequest getTripSchedulesRequest) {
        ...
        return new ResponseEntity(String.format("No trips between source stop - '%s' and destination stop - '%s' are available at this time.", getTripSchedulesRequest.getSourceStop(), getTripSchedulesRequest.getDestinationStop()),HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    public ResponseEntity<?> getTripSchedules(@RequestBody @Valid GetTripSchedulesRequest getTripSchedulesRequest) {
        ...
        if (!tripScheduleDtos.isEmpty()) {
        	return new ResponseEntity(tripScheduleDtos,HttpStatus.OK);
            //return Response.ok().setPayload(tripScheduleDtos);
        }
        return new ResponseEntity(String.format("No trips between source stop - '%s' and destination stop - '%s' on date - '%s' are available at this time.", getTripSchedulesRequest.getSourceStop(), getTripSchedulesRequest.getDestinationStop(), DateUtils.formattedDate(getTripSchedulesRequest.getTripDate())),HttpStatus.NOT_FOUND);
        //return Response.notFound()
    ...
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    public ResponseEntity<?> bookTicket(@RequestBody @Valid BookTicketRequest bookTicketRequest) {
            ...
            if (tripDto.isPresent()) {
                ...
                if (tripScheduleDto.isPresent()) {
                    ...
                    if (ticketDto.isPresent()) {
                    	return new ResponseEntity(ticketDto.get(),HttpStatus.OK);
                       // return Response.ok().setPayload(ticketDto.get());
                    }
                }
            }
        }
        return new ResponseEntity("Unable to process ticket booking.",HttpStatus.BAD_REQUEST);
